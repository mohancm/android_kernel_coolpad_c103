...
int qpnp_pon_set_restart_reason(enum pon_restart_reason reason)
{
	...
	if (!pon)
		return 0;
	/*Modified by zhaochengliang for exception stats,lafite-158(ql1530) 20160121,Begin*/
	if (!pon->store_hard_reset_reason  && (reason != PON_RESTART_REASON_PANIC))
		return 0;
	/*Modified by zhaochengliang for exception stats,lafite-158(ql1530) 20160121,End*/
...
qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	/*<LAFITE-2697 xingbin add power key log for debug begin */
	printk("input_report_key key_code=%d key_value=%d\n", cfg->key_code, pon_rt_sts & pon_rt_bit);
	/* LAFITE-2697 xingbin add power key log for debug end >*/
