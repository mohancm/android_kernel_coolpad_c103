...
/*static int store_eeprom_to_file(void)
{
    struct file *fp;
    mm_segment_t fs;
    loff_t pos;
    pr_err("%s:  enter ...\n",__func__);
    fp = filp_open("/data/misc/camera/eepromback.dat",O_RDWR|O_CREAT, 0644);
    if (IS_ERR(fp)) {
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    vfs_write(fp, (char *)buf_eeprom_w, EEPROM_CAL_TOTAL_NUM, &pos);
    filp_close(fp, NULL);
    set_fs(fs);
        kfree(buf_eeprom_w);
    pr_err("%s:  leave ...\n",__func__);
